...
[10, 13, 16, 19]
\end{lstlisting}
What is going on here? In the first example, we see that \texttt{range()}
doesn't actually return a list. It returns a special object that
\emph{behaves} like a list when used in a \texttt{for} loop. But if we want a
real list, we have to coerce it --- just like when we coerce a string to an
integer by calling \texttt{int('3')}.
